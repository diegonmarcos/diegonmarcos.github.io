// Portable Vue Component - ComplexWaveVisualization
// No build step required - works in any HTML file

import { ref, onMounted, onUnmounted, computed } from 'vue';

export default {
  template: `
    <div class="pv-container" style="background: transparent;">
      <!-- CSS Animations - Namespaced with pv- -->
      <style>
        ...
        .pv-shimmer { animation: pv-shimmer 2s ease-in-out infinite; }
        .pv-glow-pulse { animation: pv-glow-pulse 1.5s ease-in-out infinite; }
        .pv-dash-flow { animation: pv-dash-flow 1s linear infinite; }
        .pv-point-pulse { animation: pv-point-pulse 0.8s ease-in-out infinite; }
        .pv-container {
          width: 100%;
          height: 100%;
          display: flex;
      ...
      <!-- Title -->
      <div class="pv-title">
        <h1 style="background: linear-gradient(to right, #a855f7, #ec4899, #22d3ee); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;">
          The Observer Effect explains why the same
        </h1>
        <h2 style="background: linear-gradient(to right, #22d3ee, #a855f7, #ec4899); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;">
      ...
      <div class="pv-values">
        <div class="text-center mb-3">
          <span class="text-xl" style="font-family: Times New Roman, serif;">
            <span style="color: #a855f7; font-style: italic;">f</span>
            <span style="color: #d1d5db;">(</span>
            <span style="color: #a855f7; font-style: italic;">t</span>
            <span style="color: #d1d5db;">) = </span>
            <span style="color: #fb923c;">{{ realPart >= 0 ? '+' : '' }}{{ realPart.toFixed(2) }}</span>
            <span style="color: #d1d5db;"> + </span>
            <span style="color: #22d3d3;">{{ imagPart.toFixed(2) }}</span>
            <span style="color: #22d3d3; font-style: italic;"> i</span>
          </span>
        </div>

        <div class="text-center">
          <span class="text-lg" style="font-family: Times New Roman, serif;">
            <span style="color: #a855f7; font-style: italic;">t</span>
            <span style="color: #9ca3af;"> = </span>
            <span style="color: #d1d5db;">{{ (time / T).toFixed(2) }}</span>
            <span style="color: #9ca3af;"> </span>
            <span style="color: #a855f7; font-style: italic;">T</span>
          </span>
        </div>
  ...
  setup() {
    ...
    // Offsets
    const backWallRe = -2.8;
    const floorIm = -2.8;
    const leftPlaneT = -1.2;
    const leftPlaneSize = 1.3;
    const bwHeight = 1.3;
    const floorWidth = 1.3;

    // Helper functions
    const envelope = (t) => Math.exp(-gamma * Math.pow(t - t0, 2));

    const project3D = (t, re, im) => {
      const scale = 38;
      const centerX = 270;
      const centerY = 220;
      const tx = -t * 50 * 0.7;
    ...
    // Computed values
    const realPart = computed(() => envelope(time.value) * Math.cos(omega * time.value));
    const imagPart = computed(() => envelope(time.value) * Math.sin(omega * time.value));
    ...
    // Paths
    const helix3DPath = computed(() => points.value.map((p, i) => {
      const proj = project3D(p.t, p.re, p.im);
      return `${i === 0 ? 'M' : 'L'} ${proj.x} ${proj.y}`;
    }).join(' '));

    const leftSpiralPath = computed(() => points.value.map((p, i) => {
      const proj = project3D(leftPlaneT, p.re, p.im);
      return `${i === 0 ? 'M' : 'L'} ${proj.x} ${proj.y}`;
    }).join(' '));

    const backWallPath = computed(() => points.value.map((p, i) => {
      const proj = project3D(p.t, backWallRe, p.im);
      return `${i === 0 ? 'M' : 'L'} ${proj.x} ${proj.y}`;
    }).join(' '));

    const floorWavePath = computed(() => points.value.map((p, i) => {
      const proj = project3D(p.t, p.re, floorIm);
      return `${i === 0 ? 'M' : 'L'} ${proj.x} ${proj.y}`;
    }).join(' '));

    // Corners
    const lpCorners = computed(() => ({
    ...
    const lpAxisImTop = computed(() => project3D(leftPlaneT, 0, 1.4));
    const lpAxisImBot = computed(() => project3D(leftPlaneT, 0, -0.3));
    const lpAxisReLeft = computed(() => project3D(leftPlaneT, -0.3, 0));
    const lpAxisReRight = computed(() => project3D(leftPlaneT, 1.0, 0));

    const bwCorners = computed(() => ({
    ...
    // Axes
    const axisOrigin = computed(() => project3D(t0, 0, 0));
    const axisImTop = computed(() => project3D(t0, 0, 1.6));
    const axisReEnd = computed(() => project3D(t0, 1.5, 0));
    const axisTStart = computed(() => project3D(0, 0, 0));
    const axisTEnd = computed(() => project3D(2.7, 0, 0));

    // Current points
    const currentHelix = computed(() => project3D(time.value, realPart.value, imagPart.value));
    const currentLeft = computed(() => project3D(leftPlaneT, realPart.value, imagPart.value));
    const currentBack = computed(() => project3D(time.value, backWallRe, imagPart.value));
    const currentFloor = computed(() => project3D(time.value, realPart.value, floorIm));

    // Angles and centers
    const bwAngle = computed(() => Math.atan2(
      bwCorners.value.tl.y - bwCorners.value.tr.y,
      bwCorners.value.tl.x - bwCorners.value.tr.x
    ) * (180 / Math.PI));

    const bwCenter = computed(() => ({
      x: (bwCorners.value.tl.x + bwCorners.value.tr.x) / 2,
      y: (bwCorners.value.tl.y + bwCorners.value.tr.y) / 2
    }));

    const flAngle = computed(() => Math.atan2(
      flCorners.value.bl.y - flCorners.value.tl.y,
      flCorners.value.bl.x - flCorners.value.tl.x
    ) * (180 / Math.PI));

    const flCenter = computed(() => ({
      x: (flCorners.value.br.x + flCorners.value.tr.x) / 2,
      y: (flCorners.value.br.y + flCorners.value.tr.y) / 2
    }));

    const lpAngle = computed(() => Math.atan2(
      lpCorners.value.tl.y - lpCorners.value.tr.y,
      lpCorners.value.tl.x - lpCorners.value.tr.x
    ) * (180 / Math.PI));

    const lpCenter = computed(() => ({
      x: (lpCorners.value.tl.x + lpCorners.value.tr.x) / 2,
      y: (lpCorners.value.tl.y + lpCorners.value.tr.y) / 2
    }));

    const helixCenter = computed(() => ({
    ...
    return {
      time, T, realPart, imagPart,
      helix3DPath, leftSpiralPath, backWallPath, floorWavePath,
      lpCorners, lpAxisImTop, lpAxisImBot, lpAxisReLeft, lpAxisReRight,
      bwCorners, flCorners,
      axisOrigin, axisImTop, axisReEnd, axisTStart, axisTEnd,
      currentHelix, currentLeft, currentBack, currentFloor,
      bwAngle, bwCenter, flAngle, flCenter, lpAngle, lpCenter, helixCenter
    };
  }
};
