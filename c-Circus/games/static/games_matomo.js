...
(function() {
  ...
  document.addEventListener('click', function(e) {
    ...
    if (target.classList.contains('game-card') || target.classList.contains('game-link')) {
      ...
      _paq.push(['trackEvent', 'Games', 'Game Select', gameName]);
    }
    ...
    // Game launch buttons
      ...
      _paq.push(['trackEvent', 'Games', 'Game Launch', game]);
    }

    // Theme toggle
    else if (target.id === 'theme-toggle') {
      _paq.push(['trackEvent', 'Games', 'Theme', 'Toggle']);
    }
    ...
    // Navigation links
      ...
      _paq.push(['trackEvent', 'Games', 'Navigation', linkText]);
    }
  });
  ...
  window.addEventListener('load', function() {
    ...
    if (gameCards.length > 0) {
      _paq.push(['trackEvent', 'Games', 'Games Available', gameCards.length.toString()]);
    }
  });
