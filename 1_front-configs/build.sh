#!/usr/bin/env bash
# ============================================================================
# front/1_front-configs — consolidator + deriver (cloud/1_front-configs parity)
# ----------------------------------------------------------------------------
# Pipeline:
...
step_populate_src() {
  ...
  # data-links — linktree's hand-authored box JSONs (skip wrappers).
  # Generated data must NOT live in src/data/ — everything here gets
  # consolidated, and a file the build rewrites each run would race the ship
  # workflow's dist commit. Build outputs go to src/data-generated/ instead.
...
step_consolidate() {
  ...
  jq -n \
    ...
    '{
      _meta: {
        generated_by: "1_front-configs/build.sh consolidate",
        timestamp:    $ts,
...
step_derive() {
  ...
  jq '{
    _meta: { generated_by: "1_front-configs/build.sh derive", schema: "front-topology/v1", project_count: (.projects | length) },
    projects: [ .projects[] | {
  ...
  jq '
    ...
    | {
        _meta: { generated_by: "1_front-configs/build.sh derive", schema: "front-deps/v1", total_services: ($svc | length) },
        node: {
          merged: {
  ...
  local cloud_dist_url="https://raw.githubusercontent.com/diegonmarcos/cloud-infra/main/1_cloud-configs/dist"
  ...
  jq -n \
    ...
    '{
      _meta: { generated_by: "1_front-configs/build.sh", timestamp: $ts, schema: "front-data/v1" },
      projects: $n_proj,
