...
(() => {
  ...
  function initCarousels() {
    ...
    const profRow = querySelector(".carousel-row:nth-of-type(1)");
    const persRow = querySelector(".carousel-row:nth-of-type(2)");
    if (!profRow || !persRow)
      return;
  ...
  function initMobileScrollSelection() {
    const professionalRow2 = querySelector(".carousel-row:nth-of-type(1)");
    const personalRow2 = querySelector(".carousel-row:nth-of-type(2)");
    if (!professionalRow2 || !personalRow2)
      return;
    const throttledSelect = throttle(
      () => selectCarouselByScroll(professionalRow2, personalRow2),
      150
      // Increased throttle to reduce layout thrashing
    );
    if (isMobileDevice()) {
