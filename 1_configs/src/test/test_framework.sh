...
[ "$(git config core.hooksPath)" = "1_configs/dist/hooks" ] \
    || fail "core.hooksPath != 1_configs/dist/hooks"
...
[ -x "1_configs/dist/hooks/pre-commit" ] || fail "pre-commit missing"
...
[ -x "$REPO_ROOT/1_configs/src/gha/scripts/_engine.sh" ] \
...
for s in front-svg-sprite.sh front-image-encoder.sh front-lighthouse-snapshot.sh \
    ...
    deployed="1_configs/dist/scripts/$s"
...
EOF
bash 1_configs/dist/scripts/front-svg-sprite.sh "$fixt" "$fixt/sprite.svg" >/dev/null \
...
EOF
bash 1_configs/dist/scripts/front-localize-assets.sh \
...
H1=$(bash 1_configs/dist/scripts/front-cache-hash.sh "$fixt3/a.txt" "$fixt3/b.txt") \
...
H2=$(bash 1_configs/dist/scripts/front-cache-hash.sh "$fixt3/a.txt" "$fixt3/b.txt")
[ "$H1" = "$H2" ] || fail "front-cache-hash.sh not deterministic ($H1 vs $H2)"
echo "alpha-changed" > "$fixt3/a.txt"
H3=$(bash 1_configs/dist/scripts/front-cache-hash.sh "$fixt3/a.txt" "$fixt3/b.txt")
...
if command -v magick >/dev/null 2>&1 || command -v convert >/dev/null 2>&1; then
    ...
    bash 1_configs/dist/scripts/front-pwa-icons.sh "$fixt4/src.png" "$fixt4/out" "#112233" >/dev/null \
...
EOF
    ln -sf "$REPO_ROOT/1_configs/src/gha/scripts/_engine.sh" "$fixt5/build.sh"
...
[ -f "1_configs/dist/scripts/front-init-data-wrap.sh" ] \
    || fail "front-init-data-wrap.sh templater not deployed"
grep -q '^# Usage:' "1_configs/dist/scripts/front-init-data-wrap.sh" \
    || fail "front-init-data-wrap.sh missing Usage block"
pass "front-init-data-wrap.sh templater deployed + documented"

grep -q 'data_wrap)' "$REPO_ROOT/1_configs/src/gha/scripts/_engine.sh" \
    || fail "_engine.sh: mod_data_wrap not registered in run_build dispatch"
grep -q '^mod_data_wrap()' "$REPO_ROOT/1_configs/src/gha/scripts/_engine.sh" \
...
{ "title": "Bar Page" }
EOF
bash 1_configs/dist/scripts/front-data-json-js-wrapper.sh "$fixt_dw/src/data" >/dev/null \
...
EOF
    ln -sf "$REPO_ROOT/1_configs/src/gha/scripts/_engine.sh" "$fixt_tsc/build.sh"
