...
# ║                                                                  ║
# ║   Source : 1_configs/src/gha/scripts/build.sh
...
cmd_deps() {
    ...
    # Node deps — 1_front-configs is the single source of truth. It consolidates every
    ...
    # Order: per-project package.json → 1_front-configs → front-deps.json → root package.json → npm.
    log "Regenerating config data (1_front-configs → front-deps.json, single source)..."
    bash "$SCRIPT_DIR/1_front-configs/build.sh" rebuild
    log "Deriving root package.json from front-deps.json..."
    local gen="$SCRIPT_DIR/1_configs/dist/scripts/front-gen-root-pkg.sh"
    [ -x "$gen" ] || gen="$SCRIPT_DIR/1_configs/src/gha/scripts/front-gen-root-pkg.sh"
    "$gen" "$SCRIPT_DIR"
...
# =============================================================================
# Config — regenerate all config data via 1_front-configs (single source of truth)
# =============================================================================

cmd_config() {
    # 1_front-configs is the single source: consolidate every project's build.json +
    ...
    log "Regenerating config data via 1_front-configs (single source)..."
    bash "$SCRIPT_DIR/1_front-configs/build.sh" rebuild
