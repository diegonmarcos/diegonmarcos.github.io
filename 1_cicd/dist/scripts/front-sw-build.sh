...
# ║                                                                  ║
# ║   Source : 1_cicd/src/scripts/front-sw-build.sh
...
#   • Testable in isolation (see 9_others/tests/test-front-sw-build.sh).
...
# Prefer the deployed copy under 1_cicd/dist/; fall back to src/
# so the engine works from a fresh checkout before `9_others/build.sh`
# has been run.
locate() {
    local name="$1"
    local p="$REPO_ROOT/1_cicd/dist/scripts/$name"
    [ -x "$p" ] && { printf '%s' "$p"; return; }
    p="$REPO_ROOT/1_cicd/src/scripts/$name"
    [ -x "$p" ] && { printf '%s' "$p"; return; }
