        ...
        for session in build-landpage-sass build-landpage-ts build-linktree build-cv-web build-myfeed build-mygames build-nexus build-cloud build-feed build-others build-health build-market build-centralbank build-mymaps build-myprofile build-mymusic build-mymovies build-mymail build-myphotos build-myanalytics build-suite; do
            if tmux has-session -t "$session" 2>/dev/null; then
                case "$session" in
                    ...
                    build-suite) _servers="${_servers}  ${GREEN}*${NC} Suite (tmux)     ${BLUE}${URL_SUITE}${NC}\n" ;;
                esac
                _count=$((_count + 1))
            fi
        done
...
print_usage() {
    ...
    printf "  ${GREEN}build-suite${NC}      # Suite - Vanilla (static copy)\n"
    printf "\n"
    ...
    printf "  ${GREEN}dev-suite${NC}        # Suite - Python :8021\n"
    printf "\n"
...
dev_all() {
    ...
    if command -v tmux >/dev/null 2>&1; then
        ...
        tmux new-session -d -s build-suite "cd $PROJECT_ROOT/b_Work_Tools/suite/1.ops && sh build.sh dev" 2>/dev/null || true
...
tui_simple() {
    ...
    while true; do
        ...
        case "$_choice_lower" in
            ...
            b20)               _cmd="build-suite"; _last_msg="Building Suite" ;;
            # Dev commands
            ...
            d20)               _cmd="dev-suite"; _last_msg="Started Suite :8021" ;;
            # Utility commands
        ...
        case "$_cmd" in
            ...
            dev-suite)     log_to_file "Starting dev: suite"; sh "$PROJECT_ROOT/b_Work_Tools/suite/1.ops/build.sh" dev >> "$LOG_FILE" 2>&1 & ;;
            ...
            build-suite)     log_to_file "Building: suite"; (sh "$PROJECT_ROOT/b_Work_Tools/suite/1.ops/build.sh" build >> "$LOG_FILE" 2>&1 && mark_build_success "b_Work_Tools/suite") & ;;
...
main() {
    ...
    case "$_action" in
        ...
        build-myanalytics)
            ...
            ;;
        build-suite)
            execute_build "b_Work_Tools/suite" "build"
            ;;
        dev)
            dev_all "$_verbose"
            ;;
        ...
        dev-myanalytics)
            ...
            ;;
        dev-suite)
            dev_single "b_Work_Tools/suite" "$URL_SUITE"
            ;;
        list)
            list_servers
            ;;
