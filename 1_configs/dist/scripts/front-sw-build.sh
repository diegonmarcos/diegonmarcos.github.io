...
# ║                                                                  ║
# ║   Source : 1_configs/src/gha/scripts/front-sw-build.sh
...
#   • Testable in isolation (see 1_configs/src/tests/test-front-sw-build.sh).
...
# Prefer the deployed copy under 1_configs/dist/; fall back to src/
# so the engine works from a fresh checkout before `1_configs/build.sh`
# has been run.
locate() {
    local name="$1"
    local p="$REPO_ROOT/1_configs/dist/scripts/$name"
    [ -x "$p" ] && { printf '%s' "$p"; return; }
    p="$REPO_ROOT/1_configs/src/gha/scripts/$name"
    [ -x "$p" ] && { printf '%s' "$p"; return; }
