import type { myAstroData, FormData, City, PlanetInfo, ZodiacInfo } from '../types';
import { ZODIAC_DATA } from '../myAstro-data';
...
function buildProfileCard(data: myAstroData, formData: FormData, city: City): HTMLElement {
...
function buildLunarCard(data: myAstroData): HTMLElement {
...
function buildElementModality(data: myAstroData): HTMLElement {
...
function buildNatalPositions(data: myAstroData): HTMLElement {
...
function buildNatalAspects(data: myAstroData): HTMLElement | null {
...
function buildTransits(data: myAstroData): HTMLElement {
  const card = el('div', 'data-card');
  const hdr = el('div', 'data-card__header');
...
function buildFutureCities(data: myAstroData): HTMLElement {
...
function buildMapSection(data: myAstroData): HTMLElement {
  ...
  lbl.appendChild(document.createTextNode('myAstrocartography Map'));
  section.appendChild(lbl);

  const mapContainer = el('div', 'myAstro-map-container');
...
export function renderDashboard(data: myAstroData, formData: FormData, city: City): void {
  ...
  const sections: (HTMLElement | null)[] = [
    ...
    (() => { const n = el('p', 'footer-note', 'myAstronomical Engine v2.0 - Real Planetary Calculations'); return n; })(),
  ];
...
function initGlobeSection(data: myAstroData): void {
