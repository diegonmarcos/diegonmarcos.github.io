...
[ "$(git config core.hooksPath)" = "0_git/dist/hooks" ] \
    || fail "core.hooksPath != 0_git/dist/hooks"
...
[ -x "0_git/dist/hooks/pre-commit" ] || fail "pre-commit missing"
...
[ -x "$REPO_ROOT/1_cicd/src/scripts/_engine.sh" ] \
...
for s in front-svg-sprite.sh front-image-encoder.sh front-lighthouse-snapshot.sh \
    ...
    deployed="1_cicd/dist/scripts/$s"
...
EOF
bash 1_cicd/dist/scripts/front-svg-sprite.sh "$fixt" "$fixt/sprite.svg" >/dev/null \
...
EOF
bash 1_cicd/dist/scripts/front-localize-assets.sh \
...
H1=$(bash 1_cicd/dist/scripts/front-cache-hash.sh "$fixt3/a.txt" "$fixt3/b.txt") \
...
H2=$(bash 1_cicd/dist/scripts/front-cache-hash.sh "$fixt3/a.txt" "$fixt3/b.txt")
[ "$H1" = "$H2" ] || fail "front-cache-hash.sh not deterministic ($H1 vs $H2)"
echo "alpha-changed" > "$fixt3/a.txt"
H3=$(bash 1_cicd/dist/scripts/front-cache-hash.sh "$fixt3/a.txt" "$fixt3/b.txt")
...
if command -v magick >/dev/null 2>&1 || command -v convert >/dev/null 2>&1; then
    ...
    bash 1_cicd/dist/scripts/front-pwa-icons.sh "$fixt4/src.png" "$fixt4/out" "#112233" >/dev/null \
...
EOF
    ln -sf "$REPO_ROOT/1_cicd/src/scripts/_engine.sh" "$fixt5/build.sh"
...
{ "title": "Bar Page" }
EOF
bash 1_cicd/dist/scripts/front-data-json-js-wrapper.sh "$fixt_dw/src/data" >/dev/null \
...
EOF
    ln -sf "$REPO_ROOT/1_cicd/src/scripts/_engine.sh" "$fixt_tsc/build.sh"
