(()=>{var y=["#e8508a","#4a8aff","#3abb78","#8a6aff","#ff8a4a","#4ac0ff","#ff4a8a","#50c878","#ffa040","#8080ff","#ff6060","#40c0c0","#c060ff","#ff9060","#60b0ff"],ve=[{name:"Ana Silva",initial:"A",color:y[0]},{name:"Bruno Costa",initial:"B",color:y[1]},{name:"Carla Souza",initial:"C",color:y[2]},{name:"Daniel Lima",initial:"D",color:y[3]},{name:"Elena Torres",initial:"E",color:y[4]},{name:"Felipe Gomes",initial:"F",color:y[5]},{name:"Gabi Santos",initial:"G",color:y[6]},{name:"Hugo Pereira",initial:"H",color:y[7]},{name:"Isa Oliveira",initial:"I",color:y[8]}],Se=[{author:"Ana Silva",initial:"A",color:y[0],text:"Hey Diego! Miss our coding sessions at the uni. We should catch up soon! Saudades demais!",time:"2 hours ago"},{author:"Bruno Costa",initial:"B",color:y[1],text:"Cara, vi que voce ta morando em Berlin agora! Que top! Vamos marcar uma call?",time:"5 hours ago"},{author:"Carla Souza",initial:"C",color:y[2],text:"Happy birthday atrasado!! Hope you had an amazing day. Your projects are looking incredible btw",time:"yesterday"},{author:"Felipe Gomes",initial:"F",color:y[5],text:"Mano, teu MyMaps ta INSANO. Lembra quando a gente fazia mapa no paint? kkkk Evoluimos!",time:"2 days ago"},{author:"Gabi Santos",initial:"G",color:y[6],text:'Diego!! Join our community "Linux Users Brazil" please! We need more devs like you there',time:"3 days ago"},{author:"Hugo Pereira",initial:"H",color:y[7],text:"Saw your Strava activities... beast mode! When did you start cycling?",time:"4 days ago"},{author:"Isa Oliveira",initial:"I",color:y[8],text:"Testimonial request sent! You were the best partner for the compiler project, forever grateful",time:"1 week ago"}],Ae=[{name:"Eu odeio acordar cedo",members:1247893,emoji:"\u{1F634}",color:"#8a6aff"},{name:"NixOS Brasil",members:3420,emoji:"\u2744\uFE0F",color:"#4a8aff"},{name:"Linux Users",members:89340,emoji:"\u{1F427}",color:"#3abb78"},{name:"Cycling Addicts",members:45200,emoji:"\u{1F6B4}",color:"#ff8a4a"},{name:"Self-hosted Everything",members:12800,emoji:"\u{1F5A5}\uFE0F",color:"#4ac0ff"},{name:"Saudades do Orkut",members:892340,emoji:"\u{1F62D}",color:"#e8508a"},{name:"Maps & Geography",members:23100,emoji:"\u{1F30D}",color:"#50c878"},{name:"Berlin Brasileiros",members:8900,emoji:"\u{1F1E7}\u{1F1F7}",color:"#ffa040"},{name:"Coffee > Sleep",members:456e3,emoji:"\u2615",color:"#c08040"}],xe=[{author:"Ana Silva",date:"Dec 2023",text:"Diego is one of those rare people who can debug your code AND your life. An incredible friend, always there when you need help. 10/10 would add as friend again."},{author:"Bruno Costa",date:"Nov 2023",text:"Known Diego since university. The guy literally automated everything in his life with NixOS. Respect the dedication. Best engineer I know, no cap."},{author:"Carla Souza",date:"Oct 2023",text:"The most creative developer I have ever worked with. His projects are not just functional, they are beautiful. An artist disguised as an engineer."},{author:"Elena Torres",date:"Sep 2023",text:"Diego helped me set up my entire self-hosted infrastructure. Refused to let me use any cloud subscription. A true freedom fighter of the internet!"}],Z=["#1a2840","#2a1840","#182838","#281828","#1a3028","#302818","#201838","#283018","#381828"];function Ee(){let t=document.getElementById("friends-grid");t&&ve.forEach(e=>{let s=document.createElement("div");s.className="friend-cell",s.innerHTML=`
    ...
    </a>`,$=e.saved.length?e.saved.map(o=>u(o,"\u{1F516}")).join(""):'<p class="ig-empty">Nothing saved.</p>',c=e.liked.length?e.liked.map(o=>u(o,"\u2764\uFE0F")).join(""):'<p class="ig-empty">No likes.</p>',_=o=>Number(o.match(/(\d+)(?=\.\w+$)/)?.[1]||0),n=e.posts.slice().sort((o,h)=>_(h.media)-_(o.media)),d=30,p=[...e.saved,...e.liked].map(o=>o.caption).filter(Boolean),S=n.map((o,h)=>`<a class="ig-tile" href="#" data-post-idx="${h}"><img src="${o.media}" alt="post"></a>`),x=Array.from({length:Math.max(0,d-S.length)},(o,h)=>`
    <a class="ig-tile ig-tile--post" href="#" style="background:${P(h)}">
      <span class="ig-tile__cap">${a(p[h%(p.length||1)]||"")}</span>
    ...
    <a class="ig-tile ig-tile--post" href="#" style="background:${P(h+5)}">${T}
      <span class="ig-tile__cap">${a(p[(h+2)%(p.length||1)]||"")}</span>
    ...
    <a class="ig-tile ig-tile--post" href="#" style="background:${P(h+9)}">
      ...
      </div>`).join(""):'<p class="ig-empty">No comments.</p>',M=o=>`<div class="ig-grid">${o}</div>`,N=`<div class="ig-head__avatar ig-head__avatar--ph">${a(ee(s.name))}</div>`;t.innerHTML=`
    <nav class="ig-nav">
    ...
    <div class="ig-post-modal" id="ig-post-modal">
    ...
    </div>`,t.querySelectorAll(".ig-tab, .ig-pill").forEach(o=>{o.addEventListener("click",()=>{let h=o.dataset.pane;t.querySelectorAll(".ig-tab, .ig-pill").forEach(C=>C.classList.toggle("is-active",C===o)),t.querySelectorAll(".ig-pane").forEach(C=>C.classList.toggle("is-active",C.dataset.pane===h))})});let b=t.querySelector("#ig-modal"),k=o=>{let h=o==="followers"?e.followers:e.following,C=o==="followers"?s.followers:s.following,F=Be(h,C);t.querySelector("#ig-modal-title").textContent=o==="followers"?"Followers":"Following",t.querySelector("#ig-modal-sub").textContent=`${l(C)} ${o}`,t.querySelector("#ig-modal-list").innerHTML=F.map(U=>`
      <div class="ig-row">
      ...
      </div>`).join(""),b.classList.add("is-open")};t.querySelectorAll(".ig-head__stat[data-modal]").forEach(o=>o.addEventListener("click",()=>k(o.dataset.modal)));let R=()=>b.classList.remove("is-open");t.querySelector("#ig-modal-close").addEventListener("click",R),b.addEventListener("click",o=>{o.target===b&&R()});let g=t.querySelector("#ig-post-modal"),L=t.querySelector("#ig-post-img"),q=t.querySelector("#ig-post-dots"),z=t.querySelector("#ig-post-comments"),D=[],B=0,O=()=>{L.src=D[B],q.innerHTML=D.length>1?D.map((o,h)=>`<span class="ig-post-modal__dot${h===B?" is-active":""}"></span>`).join(""):""},te=o=>{let h=n[o];h&&(D=h.media_all?.length?h.media_all:[h.media],B=0,O(),z.innerHTML='<p class="ig-empty">No per-post comment data in this export.</p>',g.classList.add("is-open"))};t.querySelectorAll(".ig-tile[data-post-idx]").forEach(o=>o.addEventListener("click",h=>{h.preventDefault(),te(Number(o.dataset.postIdx))})),t.querySelector("#ig-post-prev").addEventListener("click",()=>{B=(B-1+D.length)%D.length,O()}),t.querySelector("#ig-post-next").addEventListener("click",()=>{B=(B+1)%D.length,O()});let Y=()=>g.classList.remove("is-open");t.querySelector("#ig-post-modal-close").addEventListener("click",Y),g.addEventListener("click",o=>{o.target===g&&Y()})}var G={home:'<svg viewBox="0 0 24 24"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/></svg>',net:'<svg viewBox="0 0 24 24"><circle cx="9" cy="7" r="4"/><path d="M17 11a4 4 0 1 0-3-6.6M1 21v-2a4 4 0 0 1 4-4h8a4 4 0 0 1 4 4v2M17 15a4 4 0 0 1 4 4v2"/></svg>',jobs:'<svg viewBox="0 0 24 24"><rect x="2" y="7" width="20" height="14" rx="2"/><path d="M8 7V5a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>',msg:'<svg viewBox="0 0 24 24"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>',bell:'<svg viewBox="0 0 24 24"><path d="M18 8a6 6 0 1 0-12 0c0 7-3 9-3 9h18s-3-2-3-9M13.7 21a2 2 0 0 1-3.4 0"/></svg>',like:'<svg viewBox="0 0 24 24"><path d="M14 9V5a3 3 0 0 0-6 0v4H5a2 2 0 0 0-2 2l1 7a2 2 0 0 0 2 2h9a2 2 0 0 0 2-2l1-7a2 2 0 0 0-2-2z"/></svg>',comment:'<svg viewBox="0 0 24 24"><path d="M21 11.5a8.4 8.4 0 0 1-9 8.4 9 9 0 0 1-4-1L3 20l1.1-4.9A8.4 8.4 0 0 1 12 3a8.4 8.4 0 0 1 9 8.5z"/></svg>',repost:'<svg viewBox="0 0 24 24"><polyline points="17 1 21 5 17 9"/><path d="M3 11V9a4 4 0 0 1 4-4h14M7 23l-4-4 4-4M21 13v2a4 4 0 0 1-4 4H3"/></svg>',send:'<svg viewBox="0 0 24 24"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>'};function W(t){return a(t).replace(/(https?:\/\/\S+)/g,'<a href="$1" target="_blank" rel="noopener">$1</a>')}var je=0;function Q(t){let e=`li-clamp-${je++}`;return`<div class="li-clamp" id="${e}"><div class="li-clamp__body">${t}</div><button type="button" class="li-clamp__toggle" data-clamp="${e}">Show more</button></div>`}function ee(t){return t.split(/\s+/).filter(Boolean).slice(0,2).map(e=>e.charAt(0).toUpperCase()).join("")}function Pe(){let t=document.getElementById("li-view");if(!t)return;let e=globalThis.PORTAL_DATA?.linkedin;if(!e){t.innerHTML='<p class="li-empty">LinkedIn data not loaded.</p>';return}let s=e.profile,l=(r,T)=>`<div class="li-nav__item">${r}<span>${T}</span></div>`,v=s.photo?`<div class="li-phead__avatar li-phead__avatar--img"><img src="${s.photo}" alt="${a(s.name)}"></div>`:`<div class="li-phead__avatar" style="background:${y[3]}">${a(ee(s.name))}</div>`,u=(r,T)=>`<section class="li-card li-section"><h2 class="li-section__title">${r}</h2>${T}</section>`,$='<p class="li-need">Not in the saved profile page \u2014 add from your LinkedIn data export (Settings \u2192 Get a copy of your data).</p>',c=e.experience.length?e.experience.map(r=>`
      <div class="li-item">
      ...
      <div class="li-item">
        ...
        </a>`).join("")}</div>`:"",d=e.skills.length?`<div class="li-skills">${e.skills.map(r=>`<span class="li-skill">${a(r)}</span>`).join("")}</div>`:$,p=e.about?`<div class="li-about">${Q(W(e.about))}</div>`:$,S=e.languages.length?e.languages.map(r=>`
      <div class="li-lang">
    ...
    </div>`,t.querySelectorAll(".li-clamp").forEach(r=>{let T=r.querySelector(".li-clamp__body"),E=r.querySelector(".li-clamp__toggle");if(T.scrollHeight<=T.clientHeight+4){E.style.display="none";return}E.addEventListener("click",()=>{let H=r.classList.toggle("li-clamp--open");E.textContent=H?"Show less":"Show more"})})}var de={bell:'<svg viewBox="0 0 24 24"><path d="M18 8a6 6 0 1 0-12 0c0 7-3 9-3 9h18s-3-2-3-9M13.7 21a2 2 0 0 1-3.4 0"/></svg>',chat:'<svg viewBox="0 0 24 24"><path d="M21 11.5a8.4 8.4 0 0 1-9 8.4 9 9 0 0 1-4-1L3 20l1.1-4.9A8.4 8.4 0 0 1 12 3a8.4 8.4 0 0 1 9 8.5z"/></svg>'},me=[220,300,180,340,260,200,320,240,280];function Ne(){let t=document.getElementById("pin-view");if(!t)return;let e=globalThis.PORTAL_DATA?.pinterest,s=e?.boards??[],l=e?.profile,v=["30% 30%","75% 15%","20% 80%","80% 75%"],u=(c,_)=>`
    <div class="pin-card__grid">
      ${v.map((n,d)=>`<div class="pin-card__cell pin-card__cell--${d}" style="background-image:url('${a(c)}');background-position:${n}" role="img" aria-label="${a(_)}"></div>`).join("")}
    </div>`,$=s.map((c,_)=>{let n=me[_%me.length],d=c.cover?u(c.cover,c.name):`<div class="pin-card__ph" style="height:${n}px;background:${P(_)}">\u{1F4CC}</div>`;return`
    ...
    <div class="pin-board">${$||'<p class="pin-empty">No boards.</p>'}</div>`}function Re(){let t=document.getElementById("tid-view");if(!t)return;let e=globalThis.PORTAL_DATA?.tidal,s=e?.playlists??[],l=e?.folders??[],v=e?.profile,u=n=>{if(!n)return"";let d=Math.floor(n/3600),p=Math.round(n%3600/60);return d?`${d}h ${p}m`:`${p} min`},$=`https://tidal.com/@${a(v?.username||"diegonmarcos")}`,c=s.map((n,d)=>{let p=n.cover?`<img class="tid-card__img" src="${a(n.cover)}" alt="${a(n.name)}" loading="lazy">`:`<div class="tid-card__ph" style="background:${P(d)}">\u266B</div>`;return`
    ...
    </a>`}).join(""),_=l.map(n=>`<span class="tid-folder">${a(n.name)}<em>${n.playlists}</em></span>`).join("");t.innerHTML=`
    <nav class="tid-nav">
    ...
    <div class="tid-main">
      <header class="tid-head">
        ...
        <div class="tid-head__sub">${v?.playlists??s.length} playlists \xB7 ${v?.tracks??0} tracks</div>
      </header>
    ...
    </div>`}var J={run:"\u{1F3C3}",ride:"\u{1F6B4}",swim:"\u{1F3CA}"};function Oe(){let t=document.getElementById("str-view");if(!t)return;let e=globalThis.PORTAL_DATA?.strava,s=e?.activities??[],l=e?.profile,v=i=>i&&parseInt(i)||0,u=i=>i.split(" \u2014 ")[1]||"",$=["run","ride","swim"],c=$.reduce((i,m)=>{let f=s.filter(w=>w.type===m);return i[m]={count:f.length,distance:f.reduce((w,A)=>w+(A.distance_km||0),0),minutes:f.reduce((w,A)=>w+v(A.duration),0),elevation:f.reduce((w,A)=>w+(A.elevation_m||0),0)},i},{}),_=$.reduce((i,m)=>i+c[m].minutes,0),n=i=>`${Math.round(i/60)}h`,d=(i,m,f)=>`<div class="str-stat"><span class="str-stat__icon">${i}</span><span class="str-stat__value">${a(m)}</span><span class="str-stat__label">${f}</span></div>`,p=`
    <div class="str-dash">
      ${d("\u{1F3C3}",`${c.run.distance.toFixed(0)} km`,`${c.run.count} runs`)}
      ${d("\u{1F6B4}",`${c.ride.distance.toFixed(0)} km`,`${c.ride.count} rides`)}
      ${d("\u{1F3CA}",`${c.swim.distance.toFixed(1)} km`,`${c.swim.count} swims`)}
      ${d("\u23F1\uFE0F",n(_),"total time")}
      ${d("\u26F0\uFE0F",`${(c.run.elevation+c.ride.elevation).toLocaleString()} m`,"elevation gain")}
    </div>`,S=new Map;s.forEach(i=>{let m=Date.parse(i.date);if(!Number.isNaN(m)){let f=new Date(m).toISOString().slice(0,10);S.set(f,(S.get(f)||0)+1)}});let x=s.length?new Date(Math.max(...s.map(i=>Date.parse(i.date)).filter(i=>!Number.isNaN(i)))):new Date(0),r=53,T=new Date(x);T.setDate(T.getDate()-r*7),T.setDate(T.getDate()-T.getDay());let E=[];for(let i=0;i<r*7;i++){let m=new Date(T);m.setDate(m.getDate()+i);let f=m.toISOString().slice(0,10),w=S.get(f)||0,A=w===0?0:w===1?1:w===2?2:3;E.push(`<span class="str-cal__cell str-cal__cell--${A}" title="${f}: ${w} activit${w===1?"y":"ies"}"></span>`)}let H=`<div class="str-cal">${E.join("")}</div>`,j=new Map;s.forEach(i=>{let m=Date.parse(i.date);if(Number.isNaN(m))return;let f=new Date(m).toISOString().slice(0,10);(j.get(f)??j.set(f,[]).get(f)).push(i)});let M=(i,m)=>new Date(Date.UTC(i,m,1)).toLocaleDateString("en-US",{month:"long",year:"numeric",timeZone:"UTC"}),N=(i,m)=>{let f=new Date(Date.UTC(i,m,1)),w=new Date(Date.UTC(i,m+1,0)).getUTCDate(),A=f.getUTCDay(),V=["S","M","T","W","T","F","S"],ye=Array.from({length:A},()=>'<span class="str-month__day str-month__day--blank"></span>'),we=Array.from({length:w},(oe,Te)=>{let le=Te+1,Le=new Date(Date.UTC(i,m,le)).toISOString().slice(0,10),re=j.get(Le)||[],ke=re.map(se=>`<span class="str-month__dot" style="background:${se.type==="run"?"#fc5200":se.type==="ride"?"#0a66c2":"#00b8d9"}" title="${a(se.title)}"></span>`).join("");return`<span class="str-month__day${re.length?" has-activity":""}"><span class="str-month__num">${le}</span><span class="str-month__dots">${ke}</span></span>`});return`
    ...
    </div>`,he=s.map(U).join(""),fe=`<div class="str-head__avatar">${a(ee(l?.name||"Diego Nepomuceno Marcos"))}</div>`;t.innerHTML=`
    <nav class="str-nav">
    ...
    </div>`,t.querySelectorAll(".str-filter").forEach(i=>{i.addEventListener("click",()=>{t.querySelectorAll(".str-filter").forEach(f=>f.classList.toggle("is-active",f===i));let m=i.dataset.type;t.querySelectorAll(".str-card").forEach(f=>{f.style.display=m==="all"||f.dataset.type===m?"":"none"})})});let $e=t.querySelector("#str-month-body"),be=t.querySelector("#str-month-label"),ne=i=>{k+=i,k<0&&(k=11,b--),k>11&&(k=0,b++),be.textContent=M(b,k),$e.innerHTML=N(b,k)};t.querySelector("#str-month-prev").addEventListener("click",()=>ne(-1)),t.querySelector("#str-month-next").addEventListener("click",()=>ne(1))}function Fe(){let t=document.getElementById("yt-view");if(!t)return;let e=globalThis.PORTAL_DATA?.youtube,s=e?.videos??[],l=e?.playlists??[],v=e?.profile,u=(n,d)=>{let p=n.thumbnail?`<img class="yt-card__img" src="${a(n.thumbnail)}" alt="${a(n.title)}" loading="lazy">`:`<div class="yt-card__ph" style="background:${P(d)}">\u25B6</div>`;return`
    <a class="yt-card" href="${a(n.url||"#")}" target="_blank" rel="noopener">
      <div class="yt-card__thumb">${p}${n.duration?`<span class="yt-card__duration">${a(n.duration)}</span>`:""}</div>
    ...
    <section class="yt-shelf">
      ...
      <div class="yt-shelf__row">${n.videos.map((d,p)=>u(d,p)).join("")}</div>
    </section>`).join(""),c=s.length?`<div class="yt-grid">${s.map((n,d)=>u(n,d)).join("")}</div>`:"",_=l.length||s.length;t.innerHTML=`
    <nav class="yt-nav">
    ...
    <div class="yt-main">
      ...
      ${_?`${$}${c}`:'<p class="yt-empty">No videos or playlists yet. YouTube data loads once the channel is scraped.</p>'}
    </div>`}var K=(t=!1)=>{let e=t?"#c7cbb9":"#8fbf3f",s=t?"#c7cbb9":"#ef4a2b",l=t?"#e4e4d8":"#ffd400",v=t?"#9a9d90":"#1a1a12";return`<svg class="icq-flower" viewBox="0 0 16 16" aria-hidden="true">${Array.from({length:8},($,c)=>{let _=c*45;return`<ellipse cx="8" cy="2.9" rx="1.9" ry="2.9" fill="${c===0?s:e}" stroke="${v}" stroke-width="0.5" transform="rotate(${_} 8 8)"/>`}).join("")}<circle cx="8" cy="8" r="2.4" fill="${l}" stroke="${v}" stroke-width="0.5"/></svg>`};var pe={online:!1,away:!1,dnd:!1,offline:!0};function Ue(){let t=document.getElementById("icq-view");if(!t)return;let e=globalThis.PORTAL_DATA||{},s=e.linkedin,l=e.instagram,v=s?.profile.name||"Diego Nepomuceno Marcos",u=l?.profile.username||"diegonmarcos",$=v.split(" ")[0],c=v.split(" ").slice(1).join(" "),_=(s?.profile.location||"Berlin, Germany").split(",").map(g=>g.trim()),n=_[0]||"",d=_[_.length-1]||"",p=s?.experience?.[0],S=s?.about||l?.profile.bio||"",x=s?.skills||[],r=s?.languages||[],T="184-042-518",E=["online","online","away","online","dnd","away","offline","offline","offline"],H=ve.map((g,L)=>({...g,status:E[L%E.length]})),j=H.filter(g=>g.status!=="offline"),M=H.filter(g=>g.status==="offline"),N=g=>`<li class="icq-contact">${K(pe[g.status])}<span>${a(g.name)}</span></li>`,b=(g,L)=>L?`<div class="icq-field"><span class="icq-field__k">${a(g)}</span><span class="icq-field__v">${a(L)}</span></div>`:"",k=[{id:"main",label:"Main",body:`
        ${b("Nickname",u)}
        ${b("First Name",$)}
        ${b("Last Name",c)}
        ${b("ICQ#",T)}
        ...
        ${r.length?`<div class="icq-field"><span class="icq-field__k">Languages</span><span class="icq-field__v">${r.map(g=>a(g.name)).join(", ")}</span></div>`:""}`},{id:"work",label:"Work",body:p?`
        ${b("Company",p.company)}
        ${b("Title",p.title)}
        ${b("Since",p.dates)}
        ${b("Location",p.location||"")}`:'<p class="icq-empty">No work info.</p>'},{id:"about",label:"About",body:S?`<p class="icq-about">${a(S)}</p>`:'<p class="icq-empty">No about info.</p>'},{id:"interests",label:"Interests",body:x.length?`<div class="icq-interests">${x.map(g=>`<span class="icq-chip">${a(g)}</span>`).join("")}</div>`:'<p class="icq-empty">No interests listed.</p>'}],R='<span class="icq-win__btns"><i>_</i><i>\u25A1</i><i>\u2715</i></span>';t.innerHTML=`
    <div class="icq-desk">
      <!-- Contact list window -->
    ...
    </div>`,t.querySelectorAll(".icq-tab").forEach(g=>{g.addEventListener("click",()=>{let L=g.dataset.icqPane;t.querySelectorAll(".icq-tab").forEach(q=>q.classList.toggle("is-active",q===g)),t.querySelectorAll(".icq-pane").forEach(q=>q.classList.toggle("is-active",q.dataset.icqPane===L))})})}function Ge(){let t=document.getElementById("me-view");if(!t)return;let e=globalThis.PORTAL_DATA||{},s=e.instagram,l=e.linkedin,v=l?.profile.name||s?.profile.name||"Diego Nepomuceno Marcos",u=l?.profile.headline||"",$=l?.profile.location||"",c=s?.profile.bio||"",_=globalThis.PORTAL_DATA?.tidal,n=[{theme:"linkedin",label:"LinkedIn",meta:l?`${l.profile.connections} connections \xB7 ${l.profile.followers.toLocaleString()} followers`:"profile",color:"#0a66c2"},{theme:"instagram",label:"Instagram",meta:s?`${s.profile.followers.toLocaleString()} followers \xB7 ${s.profile.posts} post${s.profile.posts===1?"":"s"}`:"profile",color:"#dc2743"},{theme:"pinterest",label:"Pinterest",meta:"boards & pins",color:"#e60023"},{theme:"tidal",label:"TIDAL",meta:_?`${_.profile.playlists} playlists`:"playlists",color:"#00ffff"},{theme:"strava",label:"Strava",meta:"activities & routes",color:"#fc5200"},{theme:"youtube",label:"YouTube",meta:"playlists & videos",color:"#ff0000"},{theme:"orkut",label:"Orkut",meta:"the classic profile",color:"#e9008c"},{theme:"icq",label:"ICQ",meta:"retro IM \xB7 user details",color:"#0a870a"}];t.innerHTML=`
    <div class="me-hub">
      <div class="me-card">
    ...
    </div>`,t.querySelectorAll(".me-link").forEach(d=>d.addEventListener("click",()=>ue(d.dataset.goto)))}var ze=["myprofile","orkut","instagram","linkedin","pinterest","tidal","strava","youtube","icq"];function Ye(t){return t==="myprofile"?"./":`${t}.html`}function X(t){document.documentElement.setAttribute("data-theme",t),document.querySelectorAll("[data-theme-btn]").forEach(e=>{e.classList.toggle("is-active",e.dataset.themeBtn===t)}),window.scrollTo(0,0)}function ue(t,e=!0){X(t),e&&history.pushState({theme:t},"",Ye(t))}function Ve(){let t=document.documentElement.dataset.theme||"myprofile";X(ze.includes(t)?t:"myprofile"),document.querySelectorAll("[data-theme-btn]").forEach(e=>{e.addEventListener("click",()=>ue(e.dataset.themeBtn))}),window.addEventListener("popstate",e=>{let s=e.state?.theme||"myprofile";X(s)})}function ge(){Ee(),Me(),qe(),De(),Ce(),He(),Pe(),Ne(),Re(),Oe(),Fe(),Ue(),Ge(),Ve(),setTimeout(()=>{document.querySelectorAll(".trust-meter__fill").forEach(t=>{t.style.transition="width 1.2s cubic-bezier(0.16, 1, 0.3, 1)"})},300)}async function _e(){let t=document.getElementById("theme-switch"),e=[...document.querySelectorAll("[data-theme-btn]")],s=document.documentElement.dataset.theme??"";e.find(d=>d.dataset.themeBtn!==s)?.click();let v=document.documentElement.dataset.theme!==s;v&&s&&X(s);let u="none",$=[],c={};try{u=navigator.serviceWorker?.controller?.scriptURL??"none",$=(await navigator.serviceWorker?.getRegistrations?.()??[]).map(p=>[p.installing&&"installing",p.waiting&&"waiting",p.active&&`active:${p.active.scriptURL}`].filter(Boolean).join(","));for(let p of await caches.keys())c[p]=(await(await caches.open(p)).keys()).length}catch{}let _="n/a";if(t){let d=t.getBoundingClientRect(),p=document.elementFromPoint(d.left+d.width/2,d.top+d.height/2);_=`<${p?.tagName}.${(p?.className||"").toString().trim()}> inNav:${t.contains(p)}`}let n={url:location.href,theme:document.documentElement.dataset.theme,navButtons:e.length,navWired:v,swController:u,swRegistrations:$,caches:c,navHitTest:_,ua:navigator.userAgent};return console.info("[mySocials debug]",JSON.stringify(n)),n}window.__debugReport=_e;document.readyState==="loading"?document.addEventListener("DOMContentLoaded",ge):ge();(/\bdebug\b/.test(location.search)||/\bdebug\b/.test(location.hash))&&setTimeout(()=>{_e()},500);})();
