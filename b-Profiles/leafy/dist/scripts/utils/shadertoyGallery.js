...
void main() {
    ...
    async activateShader(card, canvas, shader) {
        if (this.activeShader) {
            ...
            if (oldCard) {
                ...
                const oldPreview = oldCard.querySelector('.shader-preview');
                const oldPlayBtn = oldCard.querySelector('.shader-play-btn');
                if (oldPreview)
                    oldPreview.style.display = 'block';
                if (oldPlayBtn)
                    oldPlayBtn.style.display = 'flex';
            }
            this.activeShader = null;
        }
        ...
        const playBtn = card.querySelector('.shader-play-btn');
        if (playBtn) {
            playBtn.innerHTML = `
