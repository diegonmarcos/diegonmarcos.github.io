#!/bin/sh
# MyPhotos Build Script - Simple copy from src/ to dist/
# Usage: ./build.sh [build|clean|dev]

PROJECT_NAME="MyPhotos"
...
dev() {
    log_info "Starting dev server on :8019..."
    cd "${PROJECT_DIR}/src" && python3 -m http.server 8019
}
