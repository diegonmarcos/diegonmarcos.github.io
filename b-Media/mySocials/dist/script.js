(()=>{var g=["#e8508a","#4a8aff","#3abb78","#8a6aff","#ff8a4a","#4ac0ff","#ff4a8a","#50c878","#ffa040","#8080ff","#ff6060","#40c0c0","#c060ff","#ff9060","#60b0ff"],U=[{name:"Ana Silva",initial:"A",color:g[0]},{name:"Bruno Costa",initial:"B",color:g[1]},{name:"Carla Souza",initial:"C",color:g[2]},{name:"Daniel Lima",initial:"D",color:g[3]},{name:"Elena Torres",initial:"E",color:g[4]},{name:"Felipe Gomes",initial:"F",color:g[5]},{name:"Gabi Santos",initial:"G",color:g[6]},{name:"Hugo Pereira",initial:"H",color:g[7]},{name:"Isa Oliveira",initial:"I",color:g[8]}],J=[{author:"Ana Silva",initial:"A",color:g[0],text:"Hey Diego! Miss our coding sessions at the uni. We should catch up soon! Saudades demais!",time:"2 hours ago"},{author:"Bruno Costa",initial:"B",color:g[1],text:"Cara, vi que voce ta morando em Berlin agora! Que top! Vamos marcar uma call?",time:"5 hours ago"},{author:"Carla Souza",initial:"C",color:g[2],text:"Happy birthday atrasado!! Hope you had an amazing day. Your projects are looking incredible btw",time:"yesterday"},{author:"Felipe Gomes",initial:"F",color:g[5],text:"Mano, teu MyMaps ta INSANO. Lembra quando a gente fazia mapa no paint? kkkk Evoluimos!",time:"2 days ago"},{author:"Gabi Santos",initial:"G",color:g[6],text:'Diego!! Join our community "Linux Users Brazil" please! We need more devs like you there',time:"3 days ago"},{author:"Hugo Pereira",initial:"H",color:g[7],text:"Saw your Strava activities... beast mode! When did you start cycling?",time:"4 days ago"},{author:"Isa Oliveira",initial:"I",color:g[8],text:"Testimonial request sent! You were the best partner for the compiler project, forever grateful",time:"1 week ago"}],K=[{name:"Eu odeio acordar cedo",members:1247893,emoji:"\u{1F634}",color:"#8a6aff"},{name:"NixOS Brasil",members:3420,emoji:"\u2744\uFE0F",color:"#4a8aff"},{name:"Linux Users",members:89340,emoji:"\u{1F427}",color:"#3abb78"},{name:"Cycling Addicts",members:45200,emoji:"\u{1F6B4}",color:"#ff8a4a"},{name:"Self-hosted Everything",members:12800,emoji:"\u{1F5A5}\uFE0F",color:"#4ac0ff"},{name:"Saudades do Orkut",members:892340,emoji:"\u{1F62D}",color:"#e8508a"},{name:"Maps & Geography",members:23100,emoji:"\u{1F30D}",color:"#50c878"},{name:"Berlin Brasileiros",members:8900,emoji:"\u{1F1E7}\u{1F1F7}",color:"#ffa040"},{name:"Coffee > Sleep",members:456e3,emoji:"\u2615",color:"#c08040"}],Y=[{author:"Ana Silva",date:"Dec 2023",text:"Diego is one of those rare people who can debug your code AND your life. An incredible friend, always there when you need help. 10/10 would add as friend again."},{author:"Bruno Costa",date:"Nov 2023",text:"Known Diego since university. The guy literally automated everything in his life with NixOS. Respect the dedication. Best engineer I know, no cap."},{author:"Carla Souza",date:"Oct 2023",text:"The most creative developer I have ever worked with. His projects are not just functional, they are beautiful. An artist disguised as an engineer."},{author:"Elena Torres",date:"Sep 2023",text:"Diego helped me set up my entire self-hosted infrastructure. Refused to let me use any cloud subscription. A true freedom fighter of the internet!"}],B=["#1a2840","#2a1840","#182838","#281828","#1a3028","#302818","#201838","#283018","#381828"];function X(){let a=document.getElementById("friends-grid");a&&U.forEach(e=>{let i=document.createElement("div");i.className="friend-cell",i.innerHTML=`
    ...
    </div>`),r=(e.highlights||[]).map((s,l)=>`
    <div class="ig-hl">
      <div class="ig-hl__ring"><div class="ig-hl__avatar" style="background:${E(l)}"><span class="ig-hl__emoji">${s.emoji}</span></div></div>
      <span class="ig-hl__name">${t(s.label)}</span>
    </div>`),v=[...m,...r].join(""),d=(s,l)=>`
    <a class="ig-tile" href="${t(s.url)}" target="_blank" rel="noopener" style="background:${E(s.url.length)}">
      <span class="ig-tile__cap">${t(s.caption||s.url.replace("https://www.instagram.com/",""))}</span>
      <span class="ig-tile__badge">${l}</span>
    </a>`,c=e.saved.length?e.saved.map(s=>d(s,"\u{1F516}")).join(""):'<p class="ig-empty">Nothing saved.</p>',w=e.liked.length?e.liked.map(s=>d(s,"\u2764\uFE0F")).join(""):'<p class="ig-empty">No likes.</p>',h=30,_=[...e.saved,...e.liked].map(s=>s.caption).filter(Boolean),L=e.posts.map(s=>`<a class="ig-tile" href="#"><img src="${s.media}" alt="post"></a>`),x=Array.from({length:Math.max(0,h-L.length)},(s,l)=>`
    <a class="ig-tile ig-tile--post" href="#" style="background:${E(l)}">
      <span class="ig-tile__cap">${t(_[l%(_.length||1)]||"")}</span>
    </a>`),T=L.length||x.length?[...L,...x].join(""):'<p class="ig-empty">No posts yet.</p>',k='<svg class="ig-tile__ov" viewBox="0 0 24 24"><polygon points="10 8 16 12 10 16 10 8" fill="currentColor" stroke="none"/></svg>',o=Array.from({length:12},(s,l)=>`
    <a class="ig-tile ig-tile--post" href="#" style="background:${E(l+5)}">${k}
      <span class="ig-tile__cap">${t(_[(l+2)%(_.length||1)]||"")}</span>
    </a>`).join(""),f=Array.from({length:9},(s,l)=>`
    <a class="ig-tile ig-tile--post" href="#" style="background:${E(l+9)}">
      ...
      </div>`).join(""):'<p class="ig-empty">No comments.</p>',y=s=>`<div class="ig-grid">${s}</div>`,A=e.posts[0]?.media?`<img class="ig-head__avatar" src="${e.posts[0].media}" alt="${t(i.username)}">`:'<div class="ig-head__avatar"></div>';a.innerHTML=`
    <nav class="ig-nav">
    ...
    <div class="ig-modal" id="ig-modal">
    ...
    </div>`,a.querySelectorAll(".ig-tab, .ig-pill").forEach(s=>{s.addEventListener("click",()=>{let l=s.dataset.pane;a.querySelectorAll(".ig-tab, .ig-pill").forEach(u=>u.classList.toggle("is-active",u===s)),a.querySelectorAll(".ig-pane").forEach(u=>u.classList.toggle("is-active",u.dataset.pane===l))})});let p=a.querySelector("#ig-modal"),M=s=>{let l=s==="followers"?e.followers:e.following,u=s==="followers"?i.followers:i.following,W=te(l,u);a.querySelector("#ig-modal-title").textContent=s==="followers"?"Followers":"Following",a.querySelector("#ig-modal-sub").textContent=`${n(u)} ${s}`,a.querySelector("#ig-modal-list").innerHTML=W.map(I=>`
      <div class="ig-row">
      ...
      </div>`).join(""),p.classList.add("is-open")};a.querySelectorAll(".ig-head__stat[data-modal]").forEach(s=>s.addEventListener("click",()=>M(s.dataset.modal)));let S=()=>p.classList.remove("is-open");a.querySelector("#ig-modal-close").addEventListener("click",S),p.addEventListener("click",s=>{s.target===p&&S()})}var q={home:'<svg viewBox="0 0 24 24"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/></svg>',net:'<svg viewBox="0 0 24 24"><circle cx="9" cy="7" r="4"/><path d="M17 11a4 4 0 1 0-3-6.6M1 21v-2a4 4 0 0 1 4-4h8a4 4 0 0 1 4 4v2M17 15a4 4 0 0 1 4 4v2"/></svg>',jobs:'<svg viewBox="0 0 24 24"><rect x="2" y="7" width="20" height="14" rx="2"/><path d="M8 7V5a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>',msg:'<svg viewBox="0 0 24 24"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>',bell:'<svg viewBox="0 0 24 24"><path d="M18 8a6 6 0 1 0-12 0c0 7-3 9-3 9h18s-3-2-3-9M13.7 21a2 2 0 0 1-3.4 0"/></svg>',like:'<svg viewBox="0 0 24 24"><path d="M14 9V5a3 3 0 0 0-6 0v4H5a2 2 0 0 0-2 2l1 7a2 2 0 0 0 2 2h9a2 2 0 0 0 2-2l1-7a2 2 0 0 0-2-2z"/></svg>',comment:'<svg viewBox="0 0 24 24"><path d="M21 11.5a8.4 8.4 0 0 1-9 8.4 9 9 0 0 1-4-1L3 20l1.1-4.9A8.4 8.4 0 0 1 12 3a8.4 8.4 0 0 1 9 8.5z"/></svg>',repost:'<svg viewBox="0 0 24 24"><polyline points="17 1 21 5 17 9"/><path d="M3 11V9a4 4 0 0 1 4-4h14M7 23l-4-4 4-4M21 13v2a4 4 0 0 1-4 4H3"/></svg>',send:'<svg viewBox="0 0 24 24"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>'};function Q(a){return a.split(/\s+/).filter(Boolean).slice(0,2).map(e=>e.charAt(0).toUpperCase()).join("")}function N(a){return a.split(/\s*---\s*/).map(e=>e.trim()).filter(Boolean).map(e=>{let i=e.match(/^(@\S[\s\S]*?)\s{2,}([\s\S]*)$/),n=i?i[1].trim():"",m=(i?i[2]:e).split(/\s{2,}/).map(r=>t(r.trim())).filter(Boolean).join("<br>");return`<p class="li-para">${n?`<strong class="li-para__head">${t(n)}</strong>`:""}${m}</p>`}).join("")}function O(a){return`<div class="li-longtext"><div class="li-clamp">${a}</div><button class="li-more" type="button">\u2026see more</button></div>`}function oe(){let a=document.getElementById("li-view");if(!a)return;let e=globalThis.PORTAL_DATA?.linkedin;if(!e){a.innerHTML='<p class="li-empty">LinkedIn data not loaded.</p>';return}let i=e.profile,n=(o,f)=>`<div class="li-nav__item">${o}<span>${f}</span></div>`,m=globalThis.PORTAL_DATA?.instagram?.posts?.[0]?.media,r=i.photo||m,v=r?`<div class="li-phead__avatar li-phead__avatar--img"><img src="${r}" alt="${t(i.name)}"></div>`:`<div class="li-phead__avatar" style="background:${g[3]}">${t(Q(i.name))}</div>`,d=(o,f)=>`<section class="li-card li-section"><h2 class="li-section__title">${o}</h2>${f}</section>`,c='<p class="li-need">Not in the saved profile page \u2014 add from your LinkedIn data export (Settings \u2192 Get a copy of your data).</p>',w=e.experience.length?e.experience.map(o=>`
      <div class="li-item">
      ...
      </div>`).join(""):c,h=e.education.length?e.education.map(o=>`
      <div class="li-item">
      ...
      </div>`).join(""):c,_=e.skills.length?`<div class="li-skills">${e.skills.map(o=>`<span class="li-skill">${t(o)}</span>`).join("")}</div>`:c,L=e.about?`<div class="li-about">${O(N(e.about))}</div>`:c,x=e.languages.length?e.languages.map(o=>`
      <div class="li-lang">
        <span class="li-lang__name">${t(o.name)}</span>
        <span class="li-lang__level">${t(o.proficiency)}</span>
      </div>`).join(""):c,T=e.projects.length?e.projects.map(o=>`
      <div class="li-item li-item--proj">
        <div>
          ...
          <div class="li-item__desc">${O(N(o.description))}</div>
        </div>
    ...
    </div>`;let k=220;a.querySelectorAll(".li-longtext").forEach(o=>{let f=o.querySelector(".li-clamp"),$=o.querySelector(".li-more");if((f.textContent||"").trim().length<=k){$.style.display="none";return}$.style.display="",$.textContent="\u2026see more",$.addEventListener("click",()=>{let A=f.classList.toggle("is-expanded");$.textContent=A?"see less":"\u2026see more"})})}var F={bell:'<svg viewBox="0 0 24 24"><path d="M18 8a6 6 0 1 0-12 0c0 7-3 9-3 9h18s-3-2-3-9M13.7 21a2 2 0 0 1-3.4 0"/></svg>',chat:'<svg viewBox="0 0 24 24"><path d="M21 11.5a8.4 8.4 0 0 1-9 8.4 9 9 0 0 1-4-1L3 20l1.1-4.9A8.4 8.4 0 0 1 12 3a8.4 8.4 0 0 1 9 8.5z"/></svg>'},R=[220,300,180,340,260,200,320,240,280];function le(){let a=document.getElementById("pin-view");if(!a)return;let e=globalThis.PORTAL_DATA?.pinterest,i=e?.boards??[],n=e?.profile,m=i.map((r,v)=>{let d=R[v%R.length],c=r.cover?`<img class="pin-card__img" src="${t(r.cover)}" alt="${t(r.name)}" loading="lazy">`:`<div class="pin-card__ph" style="height:${d}px;background:${E(v)}">\u{1F4CC}</div>`;return`
    <a class="pin-card" href="https://www.pinterest.com/${t(n?.username||"diegonmarcos")}/${t(r.name.toLowerCase().replace(/[^a-z0-9]+/g,"-"))}/" target="_blank" rel="noopener">
      <div class="pin-card__media">
        ${c}
        <div class="pin-card__overlay"><span class="pin-card__save">${r.pins} pins</span></div>
      </div>
      <div class="pin-card__title">${t(r.name)}</div>
      ${r.desc?`<div class="pin-card__meta">${t(r.desc)}</div>`:`<div class="pin-card__meta">${r.pins} pins</div>`}
    </a>`}).join("");a.innerHTML=`
    <nav class="pin-nav">
      <div class="pin-nav__inner">
        <span class="pin-nav__logo">P</span>
        <a href="#" class="pin-nav__tab pin-nav__tab--active">Boards</a>
        <a href="https://www.pinterest.com/${t(n?.username||"diegonmarcos")}/" target="_blank" rel="noopener" class="pin-nav__tab">Profile</a>
        <div class="pin-nav__search"><input placeholder="Search for ideas"></div>
        <div class="pin-nav__icons">${F.bell}${F.chat}<span class="pin-nav__avatar" style="background:${g[3]}">D</span></div>
      </div>
    </nav>
    <div class="pin-head">@${t(n?.username||"diegonmarcos")} \xB7 <strong>${n?.boards??i.length}</strong> boards \xB7 <strong>${n?.pins??0}</strong> pins</div>
    <div class="pin-board">${m||'<p class="pin-empty">No boards.</p>'}</div>`}var C=(a=!1)=>{let e=a?"#c7cbb9":"#8fbf3f",i=a?"#c7cbb9":"#ef4a2b",n=a?"#e4e4d8":"#ffd400",m=a?"#9a9d90":"#1a1a12";return`<svg class="icq-flower" viewBox="0 0 16 16" aria-hidden="true">${Array.from({length:8},(v,d)=>{let c=d*45;return`<ellipse cx="8" cy="2.9" rx="1.9" ry="2.9" fill="${d===0?i:e}" stroke="${m}" stroke-width="0.5" transform="rotate(${c} 8 8)"/>`}).join("")}<circle cx="8" cy="8" r="2.4" fill="${n}" stroke="${m}" stroke-width="0.5"/></svg>`};var G={online:!1,away:!1,dnd:!1,offline:!0};function re(){let a=document.getElementById("icq-view");if(!a)return;let e=globalThis.PORTAL_DATA||{},i=e.linkedin,n=e.instagram,m=i?.profile.name||"Diego Nepomuceno Marcos",r=n?.profile.username||"diegonmarcos",v=m.split(" ")[0],d=m.split(" ").slice(1).join(" "),c=(i?.profile.location||"Berlin, Germany").split(",").map(s=>s.trim()),w=c[0]||"",h=c[c.length-1]||"",_=i?.experience?.[0],L=i?.about||n?.profile.bio||"",x=i?.skills||[],T=i?.languages||[],k="184-042-518",o=["online","online","away","online","dnd","away","offline","offline","offline"],f=U.map((s,l)=>({...s,status:o[l%o.length]})),$=f.filter(s=>s.status!=="offline"),y=f.filter(s=>s.status==="offline"),A=s=>`<li class="icq-contact">${C(G[s.status])}<span>${t(s.name)}</span></li>`,p=(s,l)=>l?`<div class="icq-field"><span class="icq-field__k">${t(s)}</span><span class="icq-field__v">${t(l)}</span></div>`:"",M=[{id:"main",label:"Main",body:`
        ${p("Nickname",r)}
        ${p("First Name",v)}
        ${p("Last Name",d)}
        ${p("ICQ#",k)}
        ${p("Email","me@diegonmarcos.com")}
        ${p("Headline",i?.profile.headline||"")}`},{id:"home",label:"Home",body:`
        ${p("City",w)}
        ${p("Country",h)}
        ${p("Homepage",i?.profile.url||"linktree.diegonmarcos.com")}
        ...
        ${p("Title",_.title)}
        ${p("Since",_.dates)}
        ${p("Location",_.location||"")}`:'<p class="icq-empty">No work info.</p>'},{id:"about",label:"About",body:L?`<p class="icq-about">${t(L)}</p>`:'<p class="icq-empty">No about info.</p>'},{id:"interests",label:"Interests",body:x.length?`<div class="icq-interests">${x.map(s=>`<span class="icq-chip">${t(s)}</span>`).join("")}</div>`:'<p class="icq-empty">No interests listed.</p>'}],S='<span class="icq-win__btns"><i>_</i><i>\u25A1</i><i>\u2715</i></span>';a.innerHTML=`
    <div class="icq-desk">
      <!-- Contact list window -->
      <div class="icq-win icq-win--list">
        <div class="icq-win__bar">${C("#ffffff")}<span class="icq-win__title">ICQ</span>${S}</div>
        <div class="icq-list">
      ...
      <div class="icq-win icq-win--details">
        <div class="icq-win__bar icq-win__bar--alt">${C("#ffffff")}<span class="icq-win__title">User Details \u2014 ${t(r)}</span>${S}</div>
        <div class="icq-detail">
          <div class="icq-detail__head">
            <div class="icq-detail__avatar">${t(Q(m))}</div>
            <div>
              <div class="icq-detail__name">${t(m)}</div>
              <div class="icq-detail__nick">"${t(r)}" \xB7 #${k}</div>
            </div>
          </div>
          <div class="icq-tabs">
    ...
    </div>`,a.querySelectorAll(".me-link").forEach(h=>h.addEventListener("click",()=>D(h.dataset.goto)))}var V="mySocials.theme",de=["myprofile","orkut","instagram","linkedin","pinterest","icq"];function D(a){document.documentElement.setAttribute("data-theme",a);try{localStorage.setItem(V,a)}catch{}document.querySelectorAll("[data-theme-btn]").forEach(e=>{e.classList.toggle("is-active",e.dataset.themeBtn===a)}),window.scrollTo(0,0)}function ge(){let a=localStorage.getItem(V);D(de.includes(a)?a:"myprofile"),document.querySelectorAll("[data-theme-btn]").forEach(e=>{e.addEventListener("click",()=>D(e.dataset.themeBtn))})}function z(){X(),Z(),ee(),ie(),ae(),ne(),oe(),le(),re(),ce(),ge(),setTimeout(()=>{document.querySelectorAll(".trust-meter__fill").forEach(a=>{a.style.transition="width 1.2s cubic-bezier(0.16, 1, 0.3, 1)"})},300)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",z):z();})();
