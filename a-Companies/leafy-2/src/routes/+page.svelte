<script lang="ts">
  ...
  const stops = cfg.stops; // each stop: title + subtitle (+ its 3D cube)
...
<!-- z10 text overlay: one panel per stop; pointer-events off so it never blocks the raycaster. -->
...
<!-- Accessible mirror of every cube face for screen readers / crawlers. -->
<nav class="sr-only" aria-label="Destinations">
  {#each stops as s}
    {#each s.cube.faces as f}
...
<style>
  ...
  .tooltip {
    ...
    font: 600 0.75rem/1.3 system-ui, sans-serif; letter-spacing: 0.06em;
