...
let linkPanel = null;
let linkPanelIcon = null;
let linkPanelTitle = null;
let linkPanelLinks = null;
let linkPanelClose = null;
let btnBack = null;
let btnHome = null;
let btnReset = null;
let btnZoomIn = null;
...
// Initialize
// -----------------------------------------------------------------------------
export function initUI(viewState, width, height) {
    view = viewState;
    canvasWidth = width;
    canvasHeight = height;
    // Get elements
...
export function setOnBack(callback) {
    onBack = callback;
}
export function setOnNodeSelect(callback) {
...
export function showTooltip(node, screenX, screenY) {
    if (!tooltip || !tooltipTitle || !tooltipSubtitle)
        return;
...
// Link Panel
// -----------------------------------------------------------------------------
export function showLinkPanel(node) {
    if (!linkPanel || !linkPanelTitle || !linkPanelLinks || !backdrop)
        return;
    currentNode = node;
    // Set accent color
...
function createLinkCard(link, accentColor) {
...
function handleSearchInput() {
    ...
    // Find matching nodes
    const matches = allNodes.filter((n) => n.label.toLowerCase().includes(query) ||
...
export function updateMinimap(nodes, edges) {
    if (!minimapCanvas || !minimapViewport || !view)
        return;
    ...
    edges.forEach((e) => {
        ...
        if (e.highlighted) {
            ...
            const gradient = ctx.createLinearGradient(e.source.x * scale + offsetX, e.source.y * scale + offsetY, e.target.x * scale + offsetX, e.target.y * scale + offsetY);
...
// -----------------------------------------------------------------------------
export function focusOnNodeWithUI(node) {
    if (!view)
        return;
