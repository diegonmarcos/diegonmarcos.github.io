...
type PageType = 'landing' | 'vc' | 'advisory' | 'consulting';
type ModalType = 'strategy' | 'growth' | 'operations' | 'technology' | null;
...
const mobileHomeBtn = document.getElementById('mobile-home-btn') as HTMLButtonElement;
const navButtons = document.querySelectorAll<HTMLButtonElement>('.nav-btn');
const pathCards = document.querySelectorAll<HTMLElement>('.path-card');
...
const modalImage = document.getElementById('modal-image') as HTMLElement;
...
const modalServices = document.getElementById('modal-services') as HTMLElement;
const modalDeliverables = document.getElementById('modal-deliverables') as HTMLElement;
 ...
 */
function updateHeader(page: PageType): void {
 ...
 */
function switchToPage(page: PageType, pushState: boolean = true): void {
...
function openModal(type: ModalType): void {
    if (!type || !modalContent[type]) return;
...
function initNavigation(): void {
    ...
    navButtons.forEach(btn => {
        btn.addEventListener('click', () => {
            const page = btn.dataset.page as PageType;
    ...
    pathCards.forEach(card => {
        card.addEventListener('click', () => {
            const page = card.dataset.page as PageType;
            if (page) switchToPage(page);
        });
    });
...
function initModals(): void {
    ...
    serviceCards.forEach(card => {
        card.addEventListener('click', () => {
            const modalType = card.dataset.modal as ModalType;
