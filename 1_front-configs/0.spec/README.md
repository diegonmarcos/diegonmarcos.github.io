# 1_front-configs · Unified Manifest Consolidator
...
```bash
~/git/front/1_front-configs/build.sh build         # populate src/ + emit dist/
~/git/front/1_front-configs/build.sh populate-src  # refresh symlinks only
~/git/front/1_front-configs/build.sh consolidate   # rebuild dist/ from current src/
~/git/front/1_front-configs/build.sh rebuild       # clean + build
~/git/front/1_front-configs/build.sh clean         # wipe dist/
```
...
own remote. `1_front-configs/` is the **forward-looking** consolidator: richer
