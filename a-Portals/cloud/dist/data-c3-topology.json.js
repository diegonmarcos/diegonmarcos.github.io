...
(function () {
  ...
  "wireguard_public": {
   ...
   "clients": {
    ...
    "termux-share": {
     ...
     "role": "client",
     "_doc": "Shareable wg-public credential held on Diego's phone. Separate keypair from 'termux' (vault dir termux-share-public/) so rotating/revoking this one doesn't affect personal admin access. Designed for handout: someone gets ONLY wg-public access (10.x.x.x/24), never wg0.",
    ...
    "health-runner": {
     ...
     "role": "client",
     "_doc": "URL-health prober (cloud-data reports container in GHA). Joins wg-public as a spoke so it can probe hub-bound services (e.g. cf-worker-bridge on 10.x.x.x:8092) \u2014 its wg0 identity (gha-runner, 10.x.x.x) only routes 10.x.x.x/24. Keypair: vault dir health-runner-public/; private key mirrored to the cloud-data repo GHA secret WG_PUBLIC_PRIVATE_KEY consumed by reports/entrypoint.sh wg1 block.",
 ...
 "services": {
  ...
  "c3-services-mcp": {
   ...
   "proxied_mcps": {
    "_note": "URLs resolved at flake build time from cloud-data services{}. Container runs in bridge mode, so 127.0.0.1 will NOT reach host-bound services \u2014 we MUST use the WG IP (10.x.x.x) of whichever VM hosts the child MCP.",
    "infra": [
     {
      "name": "cloud-cgc-mcp",
  ...
  "caddy": {
   ...
   "caddy_config": {
    "global": {
     ...
     "_doc_wg_bind_ip": "Caddy's wg0 IP on gcp-proxy (10.x.x.x) \u2014 every public *.diegonmarcos.com site block dual-binds to (0.0.0.0, wg_bind_ip). Caddy lives on gcp-proxy (the public-facing VM); reverse-proxy upstreams to internal services traverse the wg0 mesh (10.0.0.x). Engine resolveVmIpForCaddy() still prefers wg-public IPs (10.1.0.x) when both endpoints are peers \u2014 gcp-proxy is a wg-public peer (10.x.x.x) so mail-side upstreams keep using wg-public transit.",
     "wg_bind_ip": "10.x.x.x"
    },
    "on_demand_tls": {
  ...
  "caddy-public": {
   ...
   "caddy_config": {
    "global": {
     ...
     "auto_https": "disable_redirects",
     "https_port": 8443,
     "_doc_wg_bind_ip": "oci-analytics wg0 IP (10.x.x.x). caddy-public OWNS 0.0.0.0:443 via its layer4 SNI mux (mail\u2192maddy L4, pure-public\u2192local :8443 L7, rest\u2192gcp-proxy passthrough) and an internal :8443 L7 server. Non-allowlisted traffic reaches gcp-proxy over wg-public (private_forward_upstream = 10.x.x.x:443).",
     "wg_bind_ip": "10.x.x.x"
    },
    "on_demand_tls": {
     "ask_bind": "127.0.0.1:2020",
   ...
   "runtime": {
    ...
    "octocode": {
     ...
     "llm": {
      ...
      "openai_api_url": "http://10.x.x.x:3217/v1/chat/completions",
      "ollama_api_url": "http://10.x.x.x:12436",
      "api_key": "sk-dummy",
      "models": "openai:deepseek/deepseek-v4-pro",
      "health_url": "http://10.x.x.x:3217/health"
     },
     "index_repos": [
      "cloud",
      "unix",
  ...
  "my-ai-api": {
   ...
   "runtime": {
    ...
    "gateway": {
     ...
     "mattermost_url": "http://10.x.x.x:8065",
